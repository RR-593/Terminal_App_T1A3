  ...
  module Exercises
    ...
    def self.colour_astrixs(cursor, lines)
      ...
      lines.each_with_index do |line, line_i|
        coloured_lines << "\n#{Rainbow('||').color($user.colour)} " + line.chars.each_with_index do |char, char_i|
          next unless char == '*'
          ...
          if cursor == [char_i - 10, line_i] || astrix_number = $astrixs.detect { |num| num == astrix_count }
            $astrixs.concat([astrix_count]).uniq!
            colour = :green
          end

          char.sub!(/[*]/, Rainbow('*').color(colour))
        end.join('') + " #{' ' * (60 - line.chomp.chars.length + (cursor[1] == line_i ? 14 : 0)).abs}#{Rainbow('||').color($user.colour)}"
      end
      coloured_lines
    end
